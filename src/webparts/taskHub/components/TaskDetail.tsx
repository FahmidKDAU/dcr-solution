    ...
    return (
      ...
      >
        ...
        <Typography variant="body2" color="text.disabled">
          CR details and documents will appear here
        </Typography>
      </Box>
    );
  }
  ...
  return (
    <Box display="flex" flexDirection="column" height="100%">
      ...
      >
        ...
        <Tab
          label={`Scope Document${attachments.length > 0 ? ` (${attachments.length})` : ""}`}
        />
          ...
          <Box p={3}>
            ...
            {!crLoading && !cr && (
              <Typography color="text.secondary">
                No linked change request found.
              </Typography>
            )}
            {!crLoading && cr && (
              <Box display="flex" flexDirection="column" gap={2}>
                ...
                <CRField
                  ...
                  value={cr.ChangeAuthority?.Title}
                />
                <CRField
                  label="Release Authority"
                  value={cr.ReleaseAuthority?.Title}
                />
                <CRField label="Author" value={cr.Author?.Title} />
                <CRField
                  label="Reviewers"
                  value={cr.Reviewers?.map((r) => r.Title).join(", ")}
                />
                <CRField
                  label="Contributors"
                  value={cr.Contributors?.map((c) => c.Title).join(", ")}
                />
              </Box>
            )}
          </Box>
