...
const TaskTypeBadge = ({ taskType }: { taskType: string }) => {
  ...
  return (
    <Box sx={{ display: "inline-flex", backgroundColor: s.bg, color: s.color, fontSize: 11, fontWeight: 700, px: 1.25, py: 0.4, borderRadius: "4px", letterSpacing: 0.4, textTransform: "uppercase" }}>
      {taskType}
    </Box>
  );
};
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <Box component="span" sx={{ display: "inline-flex", alignItems: "center", gap: "5px", backgroundColor: s.bg, color: s.color, fontSize: 11, fontWeight: 600, px: 1, py: 0.3, borderRadius: "20px" }}>
...
const Avatar = ({ name, size = 28 }: { name: string; size?: number }) => (
  <Box sx={{ width: size, height: size, borderRadius: "50%", backgroundColor: getColor(name), display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.35, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
    {getInitials(name)}
  </Box>
);
...
const renderTaskContent = (
  ...
  switch (task.TaskType) {
    ...
    case "Document Change Process":
      return <DocumentChangeProcessTask task={task} cr={cr} currentUser={currentUser} onTaskComplete={onTaskComplete} />;
    default:
      return (
        <Box sx={{ border: "1px dashed #D2D0CE", borderRadius: 1, p: 2, textAlign: "center" }}>
...
const TaskPane = ({ task, cr, onBack, onTaskComplete, currentUser, onRefetch }: TaskPaneProps) => {
  if (!task) {
    return (
    ...
    <Box display="flex" flexDirection="column" height="100%" sx={{ backgroundColor: "#fff" }}>
      ...
      <Box px={2} pt={1.5} pb={0.5}>
        <Button
          ...
          size="small"
          onClick={onBack}
          sx={{ color: "#605E5C", fontSize: 12, fontWeight: 500, textTransform: "none", px: 1, "&:hover": { backgroundColor: "#F3F2F1", color: "#323130" } }}
        >
          All tasks
        </Button>
      </Box>
      ...
      <Box sx={{ px: 3, pt: 1, pb: 2, borderBottom: "1px solid #EDEBE9" }}>
        ...
        <Typography sx={{ fontSize: 15, fontWeight: 700, color: "#323130", lineHeight: 1.4, mb: 1 }}>
          {task.Title}
        </Typography>
      ...
      {/* ── Requestor Footer ── */}
      <Box sx={{ px: 3, py: 2, borderTop: "1px solid #EDEBE9", display: "flex", alignItems: "center", gap: 1.5 }}>
        {task.Requestor?.Title && (
          <>
            <Avatar name={task.Requestor.Title} size={26} />
            <Box>
              <Typography sx={{ fontSize: 11, color: "#A19F9D", lineHeight: 1.2 }}>Requested by</Typography>
              <Typography sx={{ fontSize: 13, fontWeight: 500, color: "#323130", lineHeight: 1.3 }}>{task.Requestor.Title}</Typography>
            </Box>
          </>
        )}
      </Box>
